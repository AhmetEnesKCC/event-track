...
export default function EventPage() {
  ...
  return (
    <div className="p-4 flex flex-col gap-4 max-w-xl mx-auto bg-white shadow rounded">
      <Link href="/" className="text-blue-600 underline">
        ← Back
      </Link>
      {event.image && <img src={event.image} alt="event" className="w-full h-48 object-cover rounded" />}
      ...
      <button onClick={toggleJoin} className={`px-4 py-2 rounded w-fit text-white ${event.participants.includes(user) ? 'bg-red-600' : 'bg-green-600'}`}>
